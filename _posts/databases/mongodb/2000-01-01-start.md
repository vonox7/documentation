...
Monitoring MongoDB logs can help you figure out performance issues on the database like figuring out which queries took too long. The MongoDB logs are either accessible via the **MongoDB dashboard** or from the from the command-line interface.

### From the command-line interface

To view the MongoDB logs from the command line interface, first find out the addon ID: 
...
Then the logs can be streamed:

```bash
$ scalingo --app my-app --addon my-app-3030 logs -f
```
